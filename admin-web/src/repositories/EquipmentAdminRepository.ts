...
export class EquipmentAdminRepository {
  private static colPath = 'equipment';

  static async list(): Promise<Equipment[]> {
    try {
      const snap = await getDocs(collection(db, this.colPath));
      return snap.docs.map(d => ({ id: d.id, ...(d.data() as Omit<Equipment, 'id'>) }));
    } catch (err) {
  ...
  static async create(data: Omit<Equipment, 'id'>): Promise<string> {
    try {
  ...
  static async update(id: string, updates: Partial<Equipment>): Promise<void> {
    try {
