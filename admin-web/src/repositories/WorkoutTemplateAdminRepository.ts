...
import { WorkoutTemplate, EditorialStatus } from '../models/types';
import { PublishingService } from '../services/publishingService';

export class WorkoutTemplateAdminRepository {
  private static colPath = 'workoutTemplates';

  static async list(): Promise<WorkoutTemplate[]> {
    try {
      const snap = await getDocs(collection(db, this.colPath));
      return snap.docs.map(d => ({ id: d.id, ...(d.data() as Omit<WorkoutTemplate, 'id'>) }));
    } catch (err) {
  ...
  static async create(data: Omit<WorkoutTemplate, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      const now = new Date().toISOString();
      const payload: Omit<WorkoutTemplate, 'id'> = {
        ...data,
        createdAt: now,
        updatedAt: now,
      };
  ...
  static async update(id: string, updates: Partial<WorkoutTemplate>): Promise<void> {
    try {
      ...
      await PublishingService.publishOrSaveContent(
        ...
        status,
        actor,
        'Atualização de modelo de treino'
      );
    } catch (err) {
